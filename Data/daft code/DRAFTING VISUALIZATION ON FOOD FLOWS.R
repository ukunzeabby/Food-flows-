...
# Filter items_sold
joined_df <- joined_df %>%
  filter(`Food.Item...English` == "onion")  # Ensure correct column name

# Separate data into same and different location dataframes
same_location <- joined_df[joined_df$source_location == joined_df$District, ]
different_location <- joined_df[joined_df$source_location != joined_df$District, ]

# Convert different location dataframe to sf object
different_location_sf <- st_as_sf(different_location)
