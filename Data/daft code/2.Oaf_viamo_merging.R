...
main_df <- main %>%
  select("survey_start","province","district","market","age","sex", "items_sold")
...
rpt_main_dsk_df <- rpt_main_dsk_df %>%
  select(
    date, time, age, Gender, Province, District, items_sold, market,
...
# Write final dataset(all OAF data) to CSV
write.csv(rpt_main_dsk_df, "OAF/megered_OAF_Data2023_march2024.csv")
...
data <- data %>%
  select("date", "time", "Province", "District", "items_sold", "source_channel","source_location", "market_transport")
...
# Filter data for the specified months
df <- df %>%
  filter(format(as.Date(date), "%Y-%m") %in% c("2024-03", "2024-03"))
...
unique(final_df$items_sold)
# Standardize items_sold
final_df$items_sold <- trimws(tolower(final_df$items_sold))

final_df<- na.omit(final_df)
# Write the final dataset(combination of VIAM +OAF DATA) to CSV
write.csv(final_df, "C:/Users/HP/Desktop/IITA/IITA 2024/LFSM/food_flows/Result/Final Data/Market_Monitoring_Round 8.csv")
