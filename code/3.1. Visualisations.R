...
#write.csv(combined_data,"Food_flows_data.csv")
...
output_file_path <- "C:/Users/HP/Desktop/IITA/IITA 2024/LFSM/food_flows/Result/plots for all rounds/Food_flows_data.csv"
...
transport_counts_plot <- ggplot(transport_counts, aes(x = market_transport, y = n)) +
  geom_bar(stat = "identity", color = "blue", fill = "blue") +  # Bars are now blue
  geom_text(aes(label = n), vjust = -0.5, size = 3, color = "blue") +  # Text color set to blue
  ggtitle("Frequency of Market Transport Modes") +
  xlab("Market Transport Mode") +
  ylab("Frequency") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, color = "blue"))  # x-axis text color set to blue
...
top_items_sold_plot <- ggplot(top_items_sold, aes(x = Food.Item...English, y = n)) +
