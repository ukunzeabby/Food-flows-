  ...
  sidebarLayout(
    sidebarPanel(
      ...
      dateRangeInput(
        "date_range",
        "Select Date Range:",
        start = min(final_data$date),
        end = max(final_data$date),
        format = "mm/yyyy",
        startview = "year"
      )
...
server <- function(input, output) {
  output$mapPlot <- renderPlot({
    ...
    same_location <- filtered_data %>%
      filter(geometry_source_location == geometry_district)
    different_location <- filtered_data %>%
      filter(geometry_source_location != geometry_district)
