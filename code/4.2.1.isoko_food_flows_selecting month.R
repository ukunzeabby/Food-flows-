...
ui <- fluidPage(
  titlePanel("Flows and Average Prices of Food Items in Rwanda"),
  sidebarLayout(
    sidebarPanel(
      selectInput("selected_item", "Select Food Item:", choices = unique(final_data$Matched_Food_Item)),
      selectInput("selected_month_year", "Select Month-Year:", choices = unique(format(final_data$date, "%m/%Y")))
    ),
    mainPanel(
      plotOutput("mapPlot")
    )
...
server <- function(input, output) {
  output$mapPlot <- renderPlot({
    # Filter data based on selected food item and month-year
    filtered_data <- final_data %>%
      filter(Matched_Food_Item == input$selected_item) %>%
      filter(format(date, "%m/%Y") == input$selected_month_year)
    ...
    # Merge average prices with the shapefile data
    rwanda_map_data <- merge(rwanda_districts, avg_price_by_district, by.x = "NAME_2", by.y = "District", all.x = TRUE)
    
    # Filter data for same and different locations
    same_location <- filtered_data %>% filter(geometry_source_location == geometry_district)
    different_location <- filtered_data %>% filter(geometry_source_location != geometry_district)
    ...
    plot <- ggplot() +
      ...
      labs(title = paste("Flows and Average Prices of", input$selected_item, "in Rwanda")) +
      theme_minimal() +
